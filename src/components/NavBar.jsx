...
export function NavBar() {
  ...
  return (
    <Box
      ref={ref}
      className="fixed px-[5%] md:px-[15%] top-0 left-0 right-0 w-screen border-b border-white/8 bg-neutral-950/75! backdrop-blur-lg z-20"
    >
      <Box className="w-full py-3 grid grid-cols-2 lg:grid-cols-3 place-items-center">
        <h1 className="text-white first:justify-self-start text-lg font-bold text-nowrap">
          Kevin Nguy
        </h1>

        <Box className="hidden gap-2 lg:flex">
          {sections.map((section) => (
            <Button
              className={`font-semibold! rounded-none! ${
                section === currentSection
                  ? "text-[14px]! text-white! border-b-2!"
              ...
              onClick={() => {
                ...
                const scrollPos =
                  el.getBoundingClientRect().y + window.scrollY - navBarHeight;
        ...
        <Box className="last:justify-self-end flex justify-end gap-4">
          <Button
            ...
            target="_blank"
            href="https://drive.google.com/file/d/17ADWxamoleud6c1fuwy3iqPqQ7d96nK0/view?usp=sharing"
          >
            Resume
          </Button>
          <Button
